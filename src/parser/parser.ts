...
export class Parser {
  ...
  constructor(tokens: Token[]) {
    // Filter out comments - they don't affect parsing
  ...
  private parseVarDeclaration(): AST.VarDeclaration {
    ...
    if (!this.check(TokenType.Identifier)) {
      ...
      return {
        type: 'VarDeclaration',
        name: { type: 'Identifier', name: '', range: this.peek().range },
        init: null,
        range: start.range,
      };
    }
  ...
  private parseIfStatement(): AST.IfStatement {
    ...
    while (this.check(TokenType.Else) && this.checkNext(TokenType.If)) {
      ...
      this.consume(TokenType.RParen, 'Expected ")" after else if condition');
  ...
  private parseExecuteStatement(): AST.ExecuteStatement {
    ...
    // Read the subcommands - everything until the closing ')'
    ...
    this.consume(TokenType.RParen, 'Expected ")" after execute subcommands');
  ...
  private parseBlockStatement(): AST.BlockStatement {
    ...
    while (!this.check(TokenType.RBrace) && !this.isAtEnd()) {
      ...
      if (this.match(TokenType.Newline) || this.match(TokenType.Semicolon)) {
        continue;
      }
  ...
  private parseComparison(): AST.Expression {
    ...
    while (this.match(TokenType.Lt, TokenType.Gt, TokenType.LtEq, TokenType.GtEq)) {
      ...
      switch (op) {
        ...
        case TokenType.LtEq: operator = '<='; break;
        case TokenType.GtEq: operator = '>='; break;
        default: operator = '<';
      }
  ...
  private parseMultiplicative(): AST.Expression {
    ...
    while (this.match(TokenType.Star, TokenType.Slash, TokenType.Percent)) {
      ...
      let operator: '*' | '/' | '%';
      switch (op) {
        case TokenType.Star: operator = '*'; break;
  ...
  private parsePostfix(): AST.Expression {
    ...
    while (true) {
      if (this.match(TokenType.LParen)) {
        ...
        const closeParen = this.consume(TokenType.RParen, 'Expected ")" after arguments');
  ...
  private parsePrimary(): AST.Expression {
    ...
    // Identifier or special identifiers
    if (this.match(TokenType.Identifier, TokenType.DunderNamespace, TokenType.DunderMain)) {
      const token = this.previous();
      return {
        type: 'Identifier',
        name: token.value,
  ...
  private error(message: string): void {
    ...
    const location = `[${token.range.start.line}:${token.range.start.character}]`;
  ...
  private synchronize(): void {
    ...
    while (!this.isAtEnd()) {
      if (this.previous().type === TokenType.Newline || this.previous().type === TokenType.Semicolon) {
        return;
      }
